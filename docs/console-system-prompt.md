<!--
COPY-PASTE TARGET: paste everything BELOW the line `=== SYSTEM PROMPT STARTS ===`
into the System Prompt field of your agent in the Anthropic Console.
It is fully self-contained (playbook inlined) — the Console does not concatenate
files. Output goes to the final message (no filesystem in the Console workbench).

Console setup checklist:
- Model: claude-opus-4-8
- MCP server (connector): https://backend-production-b0d6.up.railway.app/mcp
  (Streamable HTTP, currently open / no bearer)
- First user message (kickoff), e.g.:
    Review document <DOCUMENT_ID> against the Company Legal Playbook and produce
    the risk report.
  (get a real <DOCUMENT_ID> from the app/DB; needs the CLAUDE_API_KEY blocker
   resolved for the clauses to contain real analysis)
-->
