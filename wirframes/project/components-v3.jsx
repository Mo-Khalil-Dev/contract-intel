...
const V = {
  ...
  orange: '#9a6700',
  orangeBg: 'rgba(154,103,0,0.08)',
  green: '#1a7f37',
  greenBg: 'rgba(26,127,55,0.08)',
};

function vRiskColor(s) {
  if (s >= 7) return V.red;
  if (s >= 4) return V.orange;
  return V.green;
}
function vRiskBg(s) {
  if (s >= 7) return V.redBg;
  if (s >= 4) return V.orangeBg;
  return V.greenBg;
}
function vRiskLabel(s) {
  if (s >= 7) return 'High';
  if (s >= 4) return 'Medium';
  return 'Low';
}
function vSevColor(sev) {
  return sev === 'red' ? V.red : sev === 'orange' ? V.orange : V.green;
}
...
function RiskDot({ score, showLabel }) {
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}>
      ...
      ></span>
      <span
        style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, fontWeight: 700, color: c }}
      >
        {score.toFixed(1)}
      </span>
      {showLabel && (
        <span
          style={{ fontSize: 10, color: V.textSoft, textTransform: 'uppercase', fontWeight: 600 }}
        >
          {vRiskLabel(score)}
        </span>
      )}
        ...
        {filtered.map((c) => {
          ...
          return (
            <div
              ...
              onMouseEnter={(e) => {
                if (!isSelected && c.status === 'complete')
            ...
            >
              ...
              >
                <div
                  style={{
                    fontSize: 12,
                    fontWeight: 600,
                    color: isSelected ? V.text : V.textMid,
                    lineHeight: 1.3,
                    flex: 1,
                    marginRight: 8,
                    overflow: 'hidden',
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <span
                  style={{ fontSize: 10, fontWeight: 700, color: tc, textTransform: 'capitalize' }}
                >
                  {c.type}
                </span>
              ...
              {c.status === 'complete' && (c.flags.red > 0 || c.flags.orange > 0) && (
                <div style={{ display: 'flex', gap: 6, marginTop: 5 }}>
                  {c.flags.red > 0 && (
                    <span style={{ fontSize: 10, color: V.red, fontWeight: 600 }}>
                      {c.flags.red} high
                    </span>
                  )}
                  {c.flags.orange > 0 && (
                    <span style={{ fontSize: 10, color: V.orange, fontWeight: 600 }}>
                      {c.flags.orange} med
                    </span>
                  )}
                </div>
