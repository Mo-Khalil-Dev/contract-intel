...
function RiskBar({ score, width = 60 }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <span style={{ fontSize: 11, color: c, fontFamily: MONO, minWidth: 24 }}>
        {score.toFixed(1)}
      </span>
    </div>
  );
...
function TopNav({ screen, onNav }) {
  ...
  const activeLink = links.find((l) => l.id === screen)
    ? screen
    : ['upload', 'processing', 'results', 'deepdive', 'export'].includes(screen)
      ? 'portfolio'
      : 'home';
  ...
  return (
    ...
    >
      ...
      >
        <span style={{ fontSize: 11, color: T.amber, letterSpacing: '0.12em', fontFamily: MONO }}>
          CI
        </span>
        <span style={{ fontSize: 11, color: T.inkMid, letterSpacing: '0.08em' }}>
          CONTRACTINTEL
        </span>
      </div>
      ...
      >
        {links.map((l) => (
          <button
            ...
            onMouseEnter={(e) => {
              if (activeLink !== l.id) e.currentTarget.style.color = T.inkMid;
            }}
            onMouseLeave={(e) => {
              if (activeLink !== l.id) e.currentTarget.style.color = T.inkSoft;
            }}
          >
            {l.label}
          </button>
...
function StatCard({ label, value, sub, color }) {
  return (
    <div style={{ background: T.surface, border: `1px solid ${T.border}`, padding: '14px 18px' }}>
      ...
      <div style={{ fontSize: 24, color: color || T.ink, fontFamily: MONO, lineHeight: 1.1 }}>
        {value}
      </div>
      {sub && (
        <div style={{ fontSize: 10, color: T.inkSoft, marginTop: 4, fontFamily: MONO }}>{sub}</div>
      )}
    </div>
  );
}
