...
function PortfolioV2({ onSelectContract }) {
  ...
  const riskBands = [
    ...
    {
      label: '2–4',
      value: complete.filter((c) => c.riskScore >= 2 && c.riskScore < 4).length,
      color: '#4ade80',
    },
    {
      label: '4–6',
      value: complete.filter((c) => c.riskScore >= 4 && c.riskScore < 6).length,
      color: D.yellow,
    },
    {
      label: '6–8',
      value: complete.filter((c) => c.riskScore >= 6 && c.riskScore < 8).length,
      color: D.orange,
    },
    { label: '8–10', value: complete.filter((c) => c.riskScore >= 8).length, color: D.red },
  ];

  const typeBreakdown = ['vendor', 'license', 'partnership', 'customer', 'lease', 'nda']
    .map((t) => ({
      label: t,
      value: complete.filter((c) => c.type === t).length,
      color: {
        vendor: '#818cf8',
        license: '#38bdf8',
        partnership: '#2dd4bf',
  ...
  const donutSegs = [
    ...
    {
      label: 'Med (4–7)',
      value: complete.filter((c) => c.riskScore >= 4 && c.riskScore < 7).length,
      color: D.orange,
    },
  ...
  const upcoming = RENEWALS.filter((r) => r.daysRemaining < 90).sort(
  ...
  return (
    <div style={{ flex: 1, background: D.bg, overflow: 'auto' }}>
      ...
      >
        <div>
          <h1 style={{ margin: 0, fontSize: 18, fontWeight: 800, color: D.text }}>
            Portfolio Analysis
          </h1>
          <p style={{ margin: '2px 0 0', fontSize: 12, color: D.textSoft }}>
            {complete.length} contracts analysed · Updated 2026-04-27
          </p>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <DBtn variant="secondary" small>
            Export CSV
          </DBtn>
      ...
      <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 12 }}>
          ...
          <DStatCard
            ...
            value={avgRisk}
            sub="of 10.0"
            color={dRiskColor(parseFloat(avgRisk))}
            glow={dRiskGlow(parseFloat(avgRisk))}
          />
          <DStatCard
            label="Red Flags"
            value={totalRed}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
          ...
          >
            ...
            <div style={{ fontSize: 11, color: D.textSoft, marginBottom: 16 }}>
              {complete.length} complete contracts
            </div>
          ...
          >
            ...
            <div style={{ fontSize: 11, color: D.textSoft, marginBottom: 8 }}>
              Count per risk range
            </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: 16 }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              ...
              <tbody>
                {topRisks.map((c, i) => (
                  <tr
                    key={c.id}
                    style={{ borderTop: `1px solid ${D.border}`, cursor: 'pointer' }}
                  ...
                  >
                    ...
                    <td style={{ padding: '9px 14px' }}>
                      <div style={{ display: 'flex', gap: 5 }}>
                        {c.flags.red > 0 && (
                          <span style={{ color: D.red, fontWeight: 700, fontSize: 12 }}>
                            ●{c.flags.red}
                          </span>
                        )}
                        {c.flags.orange > 0 && (
                          <span style={{ color: D.orange, fontWeight: 700, fontSize: 12 }}>
                            ●{c.flags.orange}
                          </span>
                        )}
                      </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            >
              ...
              {[
                ...
                <div key={row.label} style={{ marginBottom: 12 }}>
                  ...
                  <div
                    style={{ height: 4, background: D.border, borderRadius: 2, overflow: 'hidden' }}
                  >
                    <div
                      style={{
                        width: `${row.pct}%`,
            ...
            >
              <div style={{ fontSize: 13, fontWeight: 700, color: D.text, marginBottom: 10 }}>
                Upcoming Renewals
              </div>
              {upcoming.slice(0, 3).map((r) => (
                <div
                  ...
                  style={{
                    padding: '8px 10px',
                    marginBottom: 7,
                    background: renewColor(r) + '14',
                    border: `1px solid ${renewColor(r)}33`,
                    borderRadius: 6,
                    display: 'flex',
                    justifyContent: 'space-between',
                    alignItems: 'center',
...
function RenewalCalendarV2() {
  ...
  return (
    <div style={{ flex: 1, background: D.bg, overflow: 'auto' }}>
      <div
        style={{ padding: '18px 24px', background: D.bg2, borderBottom: `1px solid ${D.border}` }}
      >
        <h1 style={{ margin: 0, fontSize: 18, fontWeight: 800, color: D.text }}>
          Renewal Calendar
        </h1>
        <p style={{ margin: '2px 0 0', fontSize: 12, color: D.textSoft }}>
          Track upcoming contract renewals and notice periods
        </p>
      </div>
      <div
        style={{ padding: '20px 24px', display: 'grid', gridTemplateColumns: '1fr 300px', gap: 16 }}
      >
        <div>
          {/* KPI strip */}
          <div
          ...
          >
            {[
              {
                label: 'Overdue',
                count: RENEWALS.filter((r) => r.daysRemaining < 0).length,
                color: D.red,
                glow: D.redGlow,
              },
              {
                label: 'Within 30 days',
                count: RENEWALS.filter((r) => r.daysRemaining >= 0 && r.daysRemaining < 30).length,
                color: D.orange,
                glow: D.orangeGlow,
              },
              {
                label: 'Next 90 days',
                count: RENEWALS.filter((r) => r.daysRemaining >= 0 && r.daysRemaining < 90).length,
                color: D.yellow,
                glow: D.yellowGlow,
              },
            ].map((s) => (
            ...
            {[...RENEWALS]
              ...
              .map((r) => (
                <div
                  ...
                  onMouseEnter={(e) => {
                    if (selected !== r.id) e.currentTarget.style.background = D.surface2 + '88';
                  }}
                  onMouseLeave={(e) => {
                    if (selected !== r.id) e.currentTarget.style.background = 'transparent';
                  }}
                >
                  <div
                    style={{
                  ...
                  <div style={{ fontSize: 11, color: D.textSoft, marginTop: 3 }}>
                    {r.renewalDate} · {r.noticePeriod} notice · Risk score{' '}
                    <span style={{ color: dRiskColor(r.riskScore), fontWeight: 700 }}>
                      {r.riskScore}
                    </span>
                  </div>
                </div>
        ...
        <div style={{ position: 'sticky', top: 20, alignSelf: 'start' }}>
                ...
                >
                  ...
                  <div style={{ padding: 14 }}>
                    ...
                    >
                      ...
                      <DBtn variant="secondary" style={{ width: '100%', justifyContent: 'center' }}>
                        Mark Acknowledged
                      </DBtn>
                    </div>
                  </div>
          ...
          >
            <div style={{ fontSize: 12, fontWeight: 600, color: D.text, marginBottom: 8 }}>
              Export
            </div>
            <div style={{ display: 'flex', gap: 6 }}>
              <DBtn small variant="secondary" style={{ flex: 1, justifyContent: 'center' }}>
                Outlook
              </DBtn>
              <DBtn small variant="secondary" style={{ flex: 1, justifyContent: 'center' }}>
                Google Cal
        ...
        >
          {templates.map((t) => (
            ...
            >
              ...
              <div style={{ fontSize: 11, color: D.textMute, marginBottom: 12 }}>
                Uploaded {t.uploaded}
              </div>
              <div style={{ display: 'flex', gap: 6 }}>
                <DBtn small variant="secondary" style={{ flex: 1, justifyContent: 'center' }}>
          ...
          <div
            ...
            onMouseEnter={(e) => {
              ...
              e.currentTarget.style.background = D.accentGlow;
            }}
            onMouseLeave={(e) => {
          ...
          >
            ...
            <div style={{ fontSize: 12, color: D.textSoft, textAlign: 'center' }}>
              Upload new template
            </div>
          </div>
        </div>
    ...
    <div style={{ flex: 1, background: D.bg, overflow: 'auto' }}>
      ...
      <div style={{ padding: '24px' }}>
                ...
                <tbody>
                  {TEAM_MEMBERS.map((m) => (
                    <tr key={m.id} style={{ borderBottom: `1px solid ${D.border}` }}>
                      ...
                      <td style={{ padding: '10px 16px' }}>
                        <span
                          style={{
                            ...
                            fontWeight: 700,
                            color:
                              { Admin: D.accent, Reviewer: D.yellow, Viewer: D.textSoft }[m.role] ||
                              D.textSoft,
                          }}
                        >
                          {m.role}
                      ...
                      <td style={{ padding: '10px 16px', color: D.textSoft, fontSize: 12 }}>
                        {m.lastLogin}
                      </td>
                      <td style={{ padding: '10px 16px' }}>
                        {m.status === 'pending' ? (
                          <span style={{ color: D.orange, fontSize: 12, fontWeight: 600 }}>
                            ⏳ Pending
                          </span>
                        ) : (
                          <span style={{ color: D.green, fontSize: 12, fontWeight: 600 }}>
                            ● Active
                          </span>
                        )}
                      </td>
            ...
            >
              ...
              >
                <div>
                  ...
                  <div style={{ fontSize: 13, color: D.textSoft, marginTop: 2 }}>
              ...
              <div style={{ fontSize: 11, color: D.textSoft, marginTop: 5 }}>
        ...
        {(settingsTab === 'audit' || settingsTab === 'api') && (
          <div style={{ color: D.textSoft, fontSize: 14, padding: '20px 0' }}>
            {settingsTab === 'audit' ? 'Audit log' : 'API key management'} — see v1 for full detail.
          </div>
        )}
      </div>
    </div>
        ...
        <div style={{ textAlign: 'center', padding: '20px 0' }}>
          <div
            style={{
              ...
              color: D.green,
              marginBottom: 12,
              textShadow: `0 0 20px ${D.green}`,
            }}
          >
            ✓
          </div>
          <div style={{ fontSize: 16, fontWeight: 700, color: D.text, marginBottom: 6 }}>
            Upload Complete
          </div>
          <div style={{ fontSize: 13, color: D.textSoft, marginBottom: 20 }}>
        ...
        <>
          <div
            ...
            style={{
              ...
              padding: '28px 20px',
              textAlign: 'center',
              background: dragging ? D.accentGlow : D.surface2,
              cursor: 'pointer',
              marginBottom: 14,
          ...
          >
            ...
            <div style={{ fontSize: 14, fontWeight: 600, color: D.text, marginBottom: 3 }}>
              Drag & drop contracts
            </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <DBtn variant="secondary" onClick={onClose}>
              Cancel
            </DBtn>
