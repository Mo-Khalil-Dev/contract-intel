...
const W = {
  ...
  orange:  '#d97706', orangeBg: '#fffbeb', orangeBorder: '#fed7aa',
...
function TopNav({ active, onNav }) {
  ...
  return (
    <nav style={{
      background: W.navBg, height: 48, display: 'flex', alignItems: 'center',
      padding: '0 24px', gap: 0, flexShrink: 0, position: 'sticky', top: 0, zIndex: 100,
    }}>
      {/* Logo */}
      <div style={{ display:'flex', alignItems:'center', gap:8, marginRight:32 }}>
        <div style={{ width:26, height:26, borderRadius:5, background:W.accent, display:'flex', alignItems:'center', justifyContent:'center', fontSize:11, fontWeight:900, color:'#fff' }}>CI</div>
        <span style={{ fontSize:14, fontWeight:700, color:'#fff', letterSpacing:'-0.01em' }}>ContractIntel</span>
      </div>
      {/* Links */}
      <div style={{ display:'flex', gap:2, flex:1 }}>
        {links.map(l => (
          <button key={l.id} onClick={() => onNav(l.id)} style={{
            ...
            padding: '6px 12px', borderRadius: 5, fontSize: 13, fontWeight: active===l.id ? 600 : 400,
            color: active===l.id ? '#fff' : '#a1a1aa',
            backgroundColor: active===l.id ? 'rgba(255,255,255,0.1)' : 'transparent',
            transition: 'all 0.1s',
          }}
            onMouseEnter={e => { if (active!==l.id) e.currentTarget.style.color='#e4e4e7'; }}
            onMouseLeave={e => { if (active!==l.id) e.currentTarget.style.color='#a1a1aa'; }}
          >{l.label}</button>
        ))}
      </div>
      {/* User */}
      <div style={{ display:'flex', alignItems:'center', gap:8 }}>
        ...
        <div style={{ width:28, height:28, borderRadius:'50%', background:`linear-gradient(135deg,${W.accent},#7c3aed)`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:10, fontWeight:700, color:'#fff' }}>JW</div>
      </div>
    </nav>
  );
}
...
function WTypePill({ type }) {
  const colors = { vendor:'#7c3aed', license:'#0369a1', partnership:'#0f766e', customer:'#be185d', lease:'#92400e', nda:'#374151' };
  const bgs    = { vendor:'#f5f3ff', license:'#f0f9ff', partnership:'#f0fdfa', customer:'#fdf2f8', lease:'#fffbeb', nda:'#f9fafb' };
  return (
    <span style={{ background:bgs[type]||bgs.nda, color:colors[type]||colors.nda, borderRadius:4, padding:'2px 7px', fontSize:11, fontWeight:600, textTransform:'capitalize' }}>{type}</span>
  );
}
...
function WBtn({ children, variant='primary', onClick, small, disabled, style:ex }) {
  ...
  return (
    <button onClick={onClick} disabled={disabled} style={{
      ...s[variant], borderRadius:5, cursor:disabled?'not-allowed':'pointer',
      padding:small?'4px 10px':'6px 12px', fontSize:small?11:13, fontWeight:600,
      display:'inline-flex', alignItems:'center', gap:5,
...
function WModal({ title, onClose, children, width=500 }) {
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.45)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000 }}
      ...
      <div style={{ background:W.surface, border:`1px solid ${W.border}`, borderRadius:8, width, maxWidth:'95vw', maxHeight:'90vh', overflow:'auto', boxShadow:'0 20px 60px rgba(0,0,0,0.15)' }}>
        <div style={{ padding:'14px 18px', borderBottom:`1px solid ${W.border}`, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
...
function PageShell({ title, subtitle, actions, children }) {
  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column', background:W.bg, overflow:'auto' }}>
      <div style={{ padding:'20px 28px 16px', background:W.surface, borderBottom:`1px solid ${W.border}`, display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
        <div>
...
function RiskBar({ score }) {
  return (
    <div style={{ display:'flex', alignItems:'center', gap:6 }}>
      <div style={{ width:60, height:5, background:W.border, borderRadius:3, overflow:'hidden' }}>
        <div style={{ width:`${score*10}%`, height:'100%', background:wRisk(score), borderRadius:3 }}></div>
      </div>
