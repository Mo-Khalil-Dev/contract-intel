...
function DeepDiveScreen({ contractId, flagId, onBack }) {
  ...
  const marketStandards = {
    'Unlimited Liability Exposure': [
      'Liability capped at 2–3× annual fees',
      'Minimum cap of £250k',
      'IP indemnity excluded from cap',
    ],
    'One-Sided Indemnification': [
      'Mutual and reciprocal scope',
      "Proportional to each party's negligence",
      'Excludes pre-existing IP',
    ],
    'Broad IP Assignment': [
      'Custom deliverables owned by client',
      'Pre-existing IP excluded and listed',
      'Joint ownership only for R&D',
    ],
  };
  const standards = marketStandards[f.title] || [
    'Negotiate fair and balanced terms',
  ...
  return (
    ...
    >
      <div className="ci-pad-32" style={{ maxWidth: 680, margin: '0 auto', padding: '32px' }}>
        ...
        >
          <div
            style={{ width: 10, height: 10, borderRadius: '50%', background: fc, flexShrink: 0 }}
          />
          <div>
            <div
              style={{
            ...
            >
              Status: {f.status === 'open' ? 'Open — action required' : 'Accepted'}
            </div>
          </div>
        </div>
        ...
        <div style={{ marginBottom: 28 }}>
          <SectionLabel>
            The actual clause (Page {f.page}, §{f.section})
          </SectionLabel>
          <div
            style={{
              background: T.surface,
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          >
            ...
            >
              If this clause stands as-is and something goes wrong, you may have limited or no legal
              recourse. This type of clause regularly results in six-figure losses for businesses
              that don't negotiate it away upfront.
            </p>
          </div>
        </div>
            ...
            {standards.map((s, i) => (
              <div
                ...
                style={{
                  ...
                  gap: 12,
                  padding: '11px 16px',
                  borderBottom: i < standards.length - 1 ? `1px solid ${T.border}` : 'none',
                  alignItems: 'center',
                }}
              >
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                  <circle cx="8" cy="8" r="7" fill={T.greenBg} stroke={T.greenBorder} />
                  <path
                    d="M5 8l2.5 2.5L11 6"
                    stroke={T.green}
                    strokeWidth="1.5"
                ...
                <span
                  style={{ fontSize: 13, color: T.inkMid, fontFamily: "'DM Sans', sans-serif" }}
                >
                  {s}
                </span>
              </div>
        ...
        <div className="ci-actions-wrap" style={{ display: 'flex', gap: 10 }}>
          <Btn variant="success" onClick={onBack}>
...
function _buildCompareRows() {
  return [
    ...
    },
    {
      ...
      id: 'notice',
      label: 'Notice Period',
      get: (c) => _parseDays(c.noticePeriod),
    ...
    },
    {
      ...
      get: (c) => _parseEscalation(c.priceEscalation),
    ...
    },
    {
      ...
      id: 'paymentterms',
      label: 'Payment Terms',
      get: (c) => _parseDays(c.paymentTerms),
    ...
    },
    {
      ...
      id: 'liabcap',
      label: 'Liability Cap',
      get: (c) => !_flagHas(c, /unlimited liability|liability/i),
      fmt: (v) => (v ? 'Capped' : 'Unlimited / unclear'),
      kind: 'check',
      isFavourable: (v) => v === true,
    },
    {
      group: 'Key Clauses',
      id: 'mutualindem',
      label: 'Mutual Indemnification',
      get: (c) => !_flagHas(c, /one-sided|asymmet|indemn/i),
    ...
    },
    {
      group: 'Key Clauses',
      id: 'ipreten',
      label: 'IP Retained by You',
      get: (c) => !_flagHas(c, /ip|intellectual prop/i),
    ...
    },
    {
      ...
      get: (c) => !_flagHas(c, /data portability|export|portab/i),
    ...
    },
    {
      ...
      get: (c) => !_flagHas(c, /change of control/i),
...
function _scoreRow(row, contracts) {
  ...
  if (row.kind === 'check') {
    contracts.forEach((c) => {
      const v = row.get(c);
      out[c.id] = v == null ? 'na' : row.isFavourable(v) ? 'better' : 'worse';
    });
    return out;
  }
...
function _pickWinner(rows, contracts) {
  ...
  const ranked = contracts
    .map((c) => ({ c, ...tally[c.id], net: tally[c.id].better - tally[c.id].worse }))
...
function _CompareCell({ row, contract, verdict }) {
  ...
  if (verdict === 'better') {
    bg = T.greenBg;
    bdc = T.greenBorder;
    chip = <span style={{ color: T.green, fontWeight: 800 }}>✓</span>;
  } else if (verdict === 'worse') {
    bg = T.redBg;
    bdc = T.redBorder;
    chip = <span style={{ color: T.red, fontWeight: 800 }}>✗</span>;
  } else if (verdict === 'na') {
    bg = T.bg;
    bdc = T.border;
  }
  if (row.id === 'risk') {
    ...
    return (
      ...
      >
        ...
        >
          {T.riskLabel(contract.riskScore)}
        </span>
      </div>
    );
  }
    ...
    >
      <span
        style={{
          fontSize: 12,
          color: T.ink,
          fontWeight: row.emphasised ? 700 : 500,
          lineHeight: 1.4,
          overflow: 'hidden',
...
function CompareScreen({ onSelectContract }) {
  ...
  const verdicts = useMemo(() => {
    const m = {};
    rows.forEach((r) => (m[r.id] = _scoreRow(r, selected)));
    return m;
  }, [rows, selectedIds]);
    ...
    <PageShell
      ...
      actions={
        <>
          ...
          <Btn variant="secondary" size="sm" disabled={!canCompare}>
            Print
          </Btn>
        </>
      }
    >
      <div style={{ padding: '18px 32px 36px' }}>
          ...
          {selected.map((c) => (
            ...
            >
              ...
              <button
                ...
                style={{
                  background: selected.length <= 2 ? 'transparent' : T.bg,
                  border: `1px solid ${T.border}`,
                  borderRadius: 4,
                  width: 20,
                  height: 20,
        ...
        {!canCompare && (
          ...
          >
            ...
            >
              Pick at least 2 contracts to compare
            </div>
            <div style={{ fontSize: 12, color: T.inkSoft, fontFamily: "'DM Sans', sans-serif" }}>
              You can compare up to five at once.
            </div>
          </div>
        )}
          ...
          <>
            {winner ? (
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  >
                    Leads on <strong style={{ color: T.greenDark }}>{winner.better}</strong>{' '}
                    comparable terms
                    {winner.worse > 0 && (
                      <>
                        {' '}
                        against <strong style={{ color: T.red }}>{winner.worse}</strong> losses
                      </>
                    )}{' '}
                    — net advantage <strong>+{winner.net}</strong>. Lowest risk profile and most
                    favourable clause coverage in the set.
                  </div>
                </div>
                <Btn size="sm" onClick={() => onSelectContract(winner.c.id)}>
                  Open contract →
                </Btn>
              </div>
            ) : (
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div
                    style={{ fontSize: 12, color: T.inkSoft, fontFamily: "'DM Sans', sans-serif" }}
                  >
                    The selected contracts trade off across categories. Review each row to weigh
                    priorities.
                  </div>
                </div>
              </div>
                ...
                {selected.map((c, i) => {
                  ...
                  return (
                    <div
                      ...
                      style={{
                        padding: '12px 14px',
                        borderRight: i < selected.length - 1 ? `1px solid ${T.border}` : 'none',
                        background: isWinner ? T.greenBg : T.bg,
                        position: 'relative',
                      }}
                    >
                      {isWinner && (
                    ...
                    {gr.map((row, ri) => (
                      <div
                        ...
                        style={{
                          display: 'grid',
                          gridTemplateColumns: gridCols,
                          borderBottom: ri < gr.length - 1 ? `1px solid ${T.border}` : 'none',
                          background: row.emphasised ? T.blueLight : T.surface,
                        }}
                      >
                        <div
                          style={{
                        ...
                        {selected.map((c, ci) => (
                          <div
                            ...
                            style={{
                              padding: '8px 10px',
                              borderRight:
                                ci < selected.length - 1 ? `1px solid ${T.border}` : 'none',
                            }}
                          >
            ...
            >
              ...
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                ...
                <span style={{ color: T.red, fontWeight: 800, fontSize: 13 }}>✗</span>Favourable /
                unfavourable clause
              </span>
            </div>
          </>
      ...
      {showExport && (
        <Modal title="Export comparison report" onClose={() => setShowExport(false)} width={460}>
          ...
          >
            Generate a formatted report covering all {rows.length} comparison rows across the{' '}
            {selected.length} selected contracts.
          </div>
          <div style={{ display: 'grid', gap: 8, marginBottom: 18 }}>
            {[
              {
                fmt: 'PDF',
                desc: 'Formatted report with charts and full clause excerpts',
                size: '~280 KB',
              },
              {
                fmt: 'XLSX',
                desc: 'Spreadsheet with one row per term, one column per contract',
                size: '~45 KB',
              },
              {
                fmt: 'DOCX',
                desc: 'Editable document for sharing with stakeholders',
                size: '~120 KB',
              },
            ].map((f) => (
              <label
              ...
              >
                ...
                <span
                  style={{ fontSize: 10, color: T.inkMute, fontFamily: "'DM Mono', monospace" }}
                >
                  {f.size}
                </span>
              </label>
          ...
          <div style={{ display: 'flex', gap: 7, justifyContent: 'flex-end' }}>
            <Btn variant="secondary" size="sm" onClick={() => setShowExport(false)}>
              Cancel
            </Btn>
            <Btn size="sm" onClick={() => setShowExport(false)}>
              Generate report
            </Btn>
          </div>
        </Modal>
...
function _CompareScreen_OLD_UNUSED({ onSelectContract }) {
  ...
  const rows = [
    {
      ...
      winnerFn: (a, b) =>
        parseFloat(a) < parseFloat(b) ? 'A' : parseFloat(b) < parseFloat(a) ? 'B' : null,
    ...
    },
    {
      ...
      valA: cA.type,
      valB: cB.type,
      renderA: () => <TypePill type={cA.type} />,
      renderB: () => <TypePill type={cB.type} />,
      winnerFn: () => null,
    },
    {
      label: 'Value',
      valA: cA.paymentAmount || '—',
      valB: cB.paymentAmount || '—',
      winnerFn: () => null,
    },
    {
      label: 'Expiry',
      valA: cA.terminationDate || '—',
      valB: cB.terminationDate || '—',
      winnerFn: () => null,
    },
    {
      label: 'Auto-Renewal',
      valA: cA.autoRenewal || '—',
      valB: cB.autoRenewal || '—',
      winnerFn: (a, b) => (a === 'No' ? 'A' : b === 'No' ? 'B' : null),
    },
    {
      label: 'Notice Period',
      valA: cA.noticePeriod || '—',
      valB: cB.noticePeriod || '—',
      winnerFn: () => null,
    },
    {
      label: 'Payment Terms',
      valA: cA.paymentTerms || '—',
      valB: cB.paymentTerms || '—',
      winnerFn: () => null,
    },
    {
      label: 'Red Flags',
      valA: String(cA.flags.red),
      valB: String(cB.flags.red),
      winnerFn: (a, b) =>
        parseInt(a) < parseInt(b) ? 'A' : parseInt(b) < parseInt(a) ? 'B' : null,
    },
    {
      label: 'Total Flags',
      valA: String(cA.flags.red + cA.flags.orange),
      valB: String(cB.flags.red + cB.flags.orange),
      winnerFn: (a, b) =>
        parseInt(a) < parseInt(b) ? 'A' : parseInt(b) < parseInt(a) ? 'B' : null,
    },
  ];
  ...
  let winsA = 0,
    winsB = 0;
  rows.forEach((r) => {
    const w = r.winnerFn(r.valA, r.valB);
    if (w === 'A') winsA++;
    else if (w === 'B') winsB++;
  });
  ...
  return (
    <PageShell
      title="Compare contracts"
      subtitle="Select two contracts to compare their key terms side by side."
      actions={
    ...
    >
      <div className="ci-pad-32" style={{ padding: '20px 32px' }}>
        ...
        >
          {[
            { label: 'Contract A', val: idA, set: setIdA },
            { label: 'Contract B', val: idB, set: setIdB },
          ].map((sel, i) => (
            <div key={i}>
              <div
                style={{
        ...
        {overallWinner && (
          ...
          >
            <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
              <path
                d="M9 2l1.8 5.4H17L12.1 10.6l1.8 5.4L9 13l-4.9 3 1.8-5.4L1 7.4h6.2L9 2z"
                fill={T.green}
              />
            </svg>
            <span
            ...
            >
              {overallWinner === 'A'
                ? cA.name.replace(/\.[^.]+$/, '')
                : cB.name.replace(/\.[^.]+$/, '')}{' '}
              has better terms overall ({overallWinner === 'A' ? winsA : winsB} vs{' '}
              {overallWinner === 'A' ? winsB : winsA} categories won)
            </span>
          </div>
        )}
          ...
          >
            ...
            {[cA, cB].map((c) => (
              <div key={c.id} style={{ padding: '11px 16px', borderLeft: `1px solid ${T.border}` }}>
                ...
                <div
                  style={{ fontSize: 11, color: T.inkSoft, fontFamily: "'DM Sans', sans-serif" }}
                >
                  {c.parties[0]}
                </div>
              </div>
          ...
          {rows.map((row, i) => {
            ...
            return (
              ...
              >
                ...
                {[
                  { val: row.valA, render: row.renderA, w: winner === 'A' },
                  { val: row.valB, render: row.renderB, w: winner === 'B' },
                ].map((cell, ci) => (
                  <div
                    key={ci}
                    style={{
                      ...
                      background: cell.w ? T.greenBg : undefined,
                      display: 'flex',
                      alignItems: 'center',
                      gap: 8,
                      justifyContent: 'space-between',
                  ...
                  >
                    <span
                      style={{ fontSize: 13, color: T.ink, fontFamily: "'DM Sans', sans-serif" }}
                    >
                      {cell.render ? cell.render() : cell.val}
                    </span>
                    {cell.w && (
                      <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                        <circle cx="7" cy="7" r="6" fill={T.green} />
                        <path
        ...
        <div className="ci-actions-wrap" style={{ marginTop: 20, display: 'flex', gap: 10 }}>
          ...
          <Btn variant="ghost" onClick={() => onSelectContract(idA)}>
            Open Contract A →
          </Btn>
          <Btn variant="ghost" onClick={() => onSelectContract(idB)}>
            Open Contract B →
          </Btn>
        </div>
      </div>
...
function PortfolioScreen({ onSelect, onUpload, layoutVariant }) {
  ...
  const avgRisk = (
    complete.reduce((s, c) => s + c.riskScore, 0) / Math.max(1, complete.length)
  ).toFixed(1);
  ...
  const cappedCount = complete.filter((c) =>
    c.riskFlags.some((f) => f.title.toLowerCase().includes('liability') && f.severity === 'green'),
  ).length;
  ...
  const kpis = [
    { label: 'Total contracts', value: CONTRACTS.length, sub: `${complete.length} analysed` },
    {
      ...
      value: avgRisk,
      sub: 'Out of 10.0',
      color: T.riskColor(parseFloat(avgRisk)),
    },
    ...
    { label: 'Unlimited liability', value: unlimitedCount, sub: 'Contracts exposed', color: T.red },
    {
      ...
      value: urgent,
      sub: 'Within 60 days',
      color: urgent > 0 ? T.orange : T.green,
    },
  ];
  ...
  return (
    <PageShell
      title="Contracts"
      subtitle={`${complete.length} contracts on file at Northwind`}
      actions={
        <>
          <Btn variant="secondary" size="sm">
            Export CSV
          </Btn>
          <Btn size="sm" onClick={onUpload}>
            + Upload
          </Btn>
        </>
      }
    >
      <div
        className="ci-pad-32"
        style={{ padding: '20px 32px', display: 'flex', flexDirection: 'column', gap: 20 }}
      >
        {/* KPI strip */}
        <div
          className="ci-grid-5"
        ...
        >
          ...
          >
            ...
            >
              ...
              <div style={{ fontSize: 11, color: T.inkSoft, fontFamily: "'DM Sans', sans-serif" }}>
              ...
              <tbody>
                {filtered.map((c, i) => (
                  <tr
                    ...
                    onMouseEnter={(e) => {
                      if (c.status === 'complete') e.currentTarget.style.background = T.bgAlt;
                    }}
                        ...
                        >
                          <span
                            style={{
                              ...
                              borderRadius: '50%',
                              background: T.blue,
                              animation: 'ciPulse 1.2s infinite',
                            }}
                          />
                          {c.progress || 0}%
                        </span>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            >
              ...
              {unlimitedCount > 0 && (
                ...
                >
                  <strong>⚠ Action needed.</strong> {unlimitedCount} contract
                  {unlimitedCount !== 1 ? 's' : ''} with unlimited or unknown liability — escalate
                  to Head of Legal.
                </div>
              )}
            </div>
                  ...
                  return (
                    ...
                    >
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        >
                          {r.noticePeriod} notice · {r.renewalDate}
                        </div>
                      </div>
                      <span
                        style={{
...
const PB_CLAUSES = [
  ...
  },
  {
    ...
    clause: 'IP ownership',
    position: 'Custom deliverables owned by Northwind',
...
function PlaybookScreen({ onSelectContract }) {
  ...
  return (
    <PageShell
      ...
      subtitle="Our agreed positions on every key contract clause"
      actions={
        <Btn variant="secondary" size="sm">
          Export playbook
        </Btn>
      ...
      <div className="ci-pad-32" style={{ padding: '20px 32px 32px' }}>
        ...
        >
          <div style={{ flex: 1, minWidth: 280 }}>
            ...
            >
              Approved by the Board · last updated Mar 2026
            </div>
            <div
              style={{
                fontSize: 12,
            ...
            >
              Every contract above £25k is scored against these positions. Anything outside the
              acceptable range needs sign-off from Head of Legal before signature.
            </div>
          </div>
          <div
            style={{ display: 'flex', gap: 14, fontSize: 12, fontFamily: "'DM Sans', sans-serif" }}
          >
            <span>
              <strong style={{ color: T.ink, fontFamily: "'DM Mono', monospace" }}>
                {PB_CLAUSES.length}
              </strong>{' '}
              <span style={{ color: T.inkSoft }}>clauses</span>
            </span>
            <span>
              <strong style={{ color: T.ink, fontFamily: "'DM Mono', monospace" }}>
                {cats.length}
              </strong>{' '}
              <span style={{ color: T.inkSoft }}>categories</span>
            </span>
          </div>
        </div>
        ...
        >
          ...
          {contract && counts && (
            <>
              ...
              <span style={{ fontSize: 11, color: T.inkSoft, fontFamily: "'DM Mono', monospace" }}>
                        ...
                        {editingId === pc.id ? (
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                            ...
                            <div style={{ display: 'flex', gap: 6 }}>
                              <Btn size="sm" onClick={() => setEditingId(null)}>
                                Save
                              </Btn>
                              <Btn size="sm" variant="secondary" onClick={() => setEditingId(null)}>
                                Cancel
                              </Btn>
                            </div>
                          </div>
                          ...
                          <div
                            ...
                            onMouseEnter={(e) => {
                              const b = e.currentTarget.querySelector('.pbEdit');
  ...
  const formats = [
    {
      icon: '📄',
      label: 'PDF Report',
      desc: 'Formatted, branded — ready to share with stakeholders',
      cta: 'Download PDF',
    },
    {
      icon: '📊',
      label: 'Excel / CSV',
      desc: 'All raw data — for your own analysis or tracking',
      cta: 'Download Excel',
    },
    {
      icon: '📋',
      label: 'Summary only',
      desc: 'One-page overview — just the risks, quick read',
    ...
    >
      <div className="ci-pad-32" style={{ maxWidth: 580, margin: '0 auto', padding: '32px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            >
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                />
                <Btn size="sm" onClick={handleSendEmail}>
                  {emailSent ? '✓ Sent!' : 'Send'}
                </Btn>
              </div>
              <div
              ...
              >
                Recipient gets a view-only link — no account needed.
              </div>
            </div>
            ...
            >
              ...
              >
                ...
                <Btn size="sm" variant="secondary" onClick={handleCopyLink}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {formats.map((f) => (
              ...
              >
                ...
                <Btn size="sm" variant="secondary" style={{ flexShrink: 0 }}>
                  {f.cta}
                </Btn>
              </div>
            ))}
        ...
        >
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
            <path
              d="M8 2L3 4.5v4C3 11.4 5.1 14 8 14s5-2.6 5-5.5v-4L8 2z"
              stroke={T.inkSoft}
              strokeWidth="1.5"
              strokeLinejoin="round"
            />
          </svg>
          <span style={{ fontSize: 12, color: T.inkSoft, fontFamily: "'DM Sans', sans-serif" }}>
            All shared links are encrypted. You can revoke access anytime from your account
            settings.
          </span>
        </div>
      </div>
...
function RenewalsScreen() {
  ...
  return (
    <PageShell title="Renewals" subtitle="Track upcoming contract renewal and notice deadlines.">
      <div
        className="ci-pad-32 ci-renewals-grid"
        style={{
          padding: '20px 32px',
          display: 'grid',
      ...
      >
                ...
                .map((r) => (
                  <tr
                    ...
                    style={{
                      borderBottom: `1px solid ${T.border}`,
                      cursor: 'pointer',
                      background: selected === r.id ? T.bgAlt : '',
                    }}
                    onMouseEnter={(e) => {
                      if (selected !== r.id) e.currentTarget.style.background = T.bgAlt;
                    }}
                    onMouseLeave={(e) => {
        ...
        <div>
          {selected ? (
              ...
              return (
                ...
                >
                  <div
                    style={{
                      padding: '14px 18px',
                      background: T.riskBg(r.riskScore),
                      borderBottom: `1px solid ${T.border}`,
                    }}
                  >
                    <div
                  ...
                  <div style={{ padding: '14px 18px' }}>
                    {[
                      ...
                      >
                        <span style={{ color: T.inkSoft, fontFamily: "'DM Sans', sans-serif" }}>
                          {k}
                        </span>
                        <span
                          style={{
            ...
            <div style={{ color: T.inkMute, fontSize: 13, fontFamily: "'DM Sans', sans-serif" }}>
            ...
            <div style={{ maxWidth: 440 }}>
              ...
              >
                ...
                <div style={{ height: 6, background: T.border, borderRadius: 4 }}>
                  <div
                    style={{ width: '72%', height: '100%', background: T.blue, borderRadius: 4 }}
                  />
                </div>
                <div
                  style={{
                ...
                >
                  36 / 50 analyses used this month
                </div>
              </div>
            </div>
          )}
          {tab === 'audit' && (
            <div style={{ fontSize: 13, color: T.inkSoft, fontFamily: "'DM Sans', sans-serif" }}>
              Full audit log coming in v1.
            </div>
          )}
        </div>
      </div>
