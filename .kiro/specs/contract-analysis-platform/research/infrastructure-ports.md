...
interface OCRResult { pages: OCRPage[]; confidence: number }
interface OCRPage { pageNumber: number; text: string }
```
...
interface LogContext { requestId?: string; userId?: string; [key: string]: unknown }
```
  ...
  static forRoot(): DynamicModule {
    return {
      ...
      providers: [
        ...
        {
          provide: OCR_SERVICE,
          useClass: process.env.OCR_DRIVER === 'google-document-ai' ? GoogleDocumentAIOCRAdapter : MockOCRAdapter,
        },
        {
          provide: QUEUE_SERVICE,
          useClass:
            process.env.QUEUE_DRIVER === 'bullmq' ? BullMQQueueAdapter :
            process.env.QUEUE_DRIVER === 'pg-boss' ? PgBossQueueAdapter :
            MemoryQueueAdapter,
        },
        {
          provide: SECRETS_SERVICE,
          useClass: process.env.SECRETS_DRIVER === 'gcp-secret-manager' ? GCPSecretManagerAdapter : EnvSecretsAdapter,
        },
        {
